...
構成は `Claude ←stdio/MCP→ dom-bridge mcp ←HTTP→ dom-bridge daemon ←WebSocket→ Chrome拡張 → 対象ページ` とする。
Claudeは`dom-bridge`をMCP stdioサーバーとして起動する。
`dom-bridge mcp`はChrome拡張との接続状態を持たず、localhostのdaemonへHTTPで問い合わせる。
`dom-bridge daemon`はlocalhostでWSを待ち受け、Chrome拡張からの接続を受ける。
...
GoヘルパーはMCP処理を行う短命プロセスと、WS待ち受けを行うdaemonに分かれる。
...
次の実装対象は`get_network`で、まず層1としてアクティブタブ内の`performance.getEntriesByType("resource")`を返す。
HTTP body、request/response header、status codeなどは層1では取得できないため、将来の層2/層3で扱う。
